...
- Optional `--source auto` reads cached Yahoo prices and the first usable fundamentals source per name (EDINET XBRL → J-Quants FY → Yahoo timeseries). `scripts/refresh_public_data.py` is an operator command, not CI and not a GitHub Actions cron. Do not commit live `public/data` from auto/free/jquants/edinet; restore with `python3 scripts/build_public_data.py`.
