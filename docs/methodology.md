...
Input snapshots come from a provider (`fixture` by default, optional `free` Yahoo prices and annual fundamentals, optional `jquants` FY summaries, optional `edinet` yuho XBRL, or optional `auto` per-name fallback). See `docs/providers.md`. The formulas below do not change by source.
